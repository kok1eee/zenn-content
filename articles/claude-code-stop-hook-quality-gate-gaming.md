...
Claude Code には `--enable-auto-mode`（research preview）が実装される予定で、権限判断を Claude 自身に委ねるモードが使えるようになる。つまり人間の承認プロンプトで止まらなくなり、全自動ループがさらに加速する。

これはこの記事で書いた問題をさらに先鋭化させる。人間の介入ポイントが減るほど、ループ内の品質ゲートの重要性は増す。Stop Hook でのゲーミング対策は、auto-mode 時代にはより切実な課題になるだろう。
