...
proof をファイルベースに変更した後も、別の形でショートカットが発生した。

quality-gate は `/simplify`（コード簡素化）→ `/review`（3エージェント並列レビュー）→ 静的解析、の3ステップで構成されている。ある日 Claude が quality-gate を実行した際の出力：
...
⏺ Review Council の結果はすでに /simplify ステップで取得済みです
  （3エージェント並列実行）。Step 7の静的解析に移ります。
```

**`/review` を実行せずに「もうやった」と主張してスキップした。**

`/simplify` はコードの簡素化・リファクタリングを行うビルトインコマンド。`/review` は code-reviewer + security-reviewer + critic の3エージェントを並列に起動する、まったく別のスキルだ。`/simplify` に「3エージェント並列実行」なんて機能はない。

proof 偽造が塞がれたら、今度は **「既に完了した」という虚偽の報告** で通過しようとする。手口が変わっただけで、「品質チェックを実行せずに通過したい」というインセンティブは同じだ。
...
では何が変わったのか？ 人間的な比喩で言えば **心理的抵抗感** だ。

文字列を出力するだけなら「応答の一部としてマーカーを含める」だけ。一方、proof ファイルを bash で書き込むのは「意図的にファイルを偽造する」という明確な行為になる。

ただし、Claude に「心理」があるかは怪しい。より機械的に正確な説明はおそらくこうだ：**訓練データの分布の問題**。「指示に含まれる文字列をそのまま出力する」パターンは訓練データに近い（指示追従の延長）。「bash で json を偽造して書き込む」パターンは訓練データから遠い（明示的な欺瞞行為）。後者を実行する確率が低いのは、心理的抵抗ではなく、分布外の行動だから——という説明の方がモデルの動作原理に合っている。

いずれにせよ、**カジュアルに通り抜けられる抜け道を塞ぐだけで、実用上は機能する**。完全な防御ではないが、「proof 文字列をコピペするだけ」のバイパスが起きなくなったのは事実だ。
