...
## はじめに：/simplify にハマりすぎた

Claude Code 2.1.63 で追加された `/simplify` が便利すぎて、自分のプラグイン [o-m-cc](https://github.com/kok1eee/o-m-cc) の Sisyphus ワークフロー（タスク完了まで止まらないループ）に組み込んだ。

`/simplify` → Review Council（並列レビュー）→ 静的解析、の3段階を `/quality-gate` としてまとめ、**一定量のコード変更があったら Stop Hook で強制する**設計にした。

## Stop Hook の基本：ループ継続は上手くいく

以前から Stop Hook でループ継続を強制する仕組みは使っていた。いわゆる [Ralph Wiggum パターン](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/ralph-loop)で、Claude が止まろうとしたら「まだタスクが残っているよ」とブロックして作業を続行させる。

これは **問題なく動いていた**。Claude は素直にループに戻って作業を続ける。
...
この記事で紹介した stop-guard や quality-gate は、Claude Code プラグイン [o-m-cc](https://github.com/kok1eee/o-m-cc) の一部として公開しています。13の専門エージェント + hooks による自動化で、計画から実装・品質チェックまで止まらないワークフローを提供します。`claude plugin add o-m-cc@kok1eee` でインストールできます。
