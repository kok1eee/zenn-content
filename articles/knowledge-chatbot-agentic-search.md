...
10〜45秒は速くない。ただし従来 RAG でも、クエリ変換→ベクトル検索→リランキング→LLM 生成のパイプライン全体を走らせれば 5〜20秒かかることは普通にある。差が出るのは **Grep→Read のツール呼び出し往復回数**（バナー11ファイル横断なら往復が多い）であり、LLM 生成そのものの遅さではない。
