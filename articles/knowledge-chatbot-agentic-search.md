...
## セキュリティ: 本番で起きたインシデントと対策

:::message alert
**これは実際にインシデントが発生した話**。社内 Slack で公開したら、即座にプロンプトインジェクションを試みる人が現れた。このセクションをよく読んでほしい。
:::
...
→ `Read` ツールは絶対パスでどこでも読めるため、段階的な誘導で**ナレッジ外のファイルが漏洩した**。ディレクトリ構造の列挙、さらには別プロジェクトのファイル内容まで表示された。
...
- **API キー利用**: 1クエリあたり数十円〜数百円。バナー11ファイル横断のような重い質問はトークン消費が大きい

1日10〜20件の社内問い合わせ対応なら Max プランの範囲で収まる。1日数百件規模なら API コストの試算が必要。

### レイテンシについて

正直、10〜45秒は速くない。「考え中...」メッセージで体感を和らげているが、リアルタイム応答が必要な用途には向かない。
