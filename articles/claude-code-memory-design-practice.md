...
## auto-memory が解決しない課題

auto-memory は優秀だが、**そもそも解決すべき課題ではないもの**がある。それは「セッション状態の保存」だ。

### auto-memory の役割を正しく理解する

auto-memory が覚えるのは**長期知識**。「このプロジェクトでは pnpm を使う」「API は /api/v2 プレフィックス」といった、セッションをまたいで使える知識。

一方で「今このセッションで認証機能のリファクタリング中、JWT 検証でハマっている」というのは**セッション状態**。これは auto-memory に入れるべき情報ではない（すぐ陳腐化する）。

```
auto-memory（MEMORY.md）に入るべきもの:
  ✅ ビルドコマンドは npm run build
  ✅ テストは vitest を使う
  ✅ APIは /api/v2 プレフィックス

auto-memory に入れるべきでないもの:
  ❌ 昨日のセッションで何をやったか → セッション状態
  ❌ どこまで進んだか → セッション状態
  ❌ 次に何をすべきか → セッション状態
```

これは auto-memory の「限界」ではなく、**そもそも別レイヤーの課題**。

### compaction でセッション状態が消える
...
auto-memory はここでは助けてくれない。auto-memory が覚えるのは「パターン」であって「今やっていること」ではないから。セッション状態は別の仕組みで保存する必要がある。
