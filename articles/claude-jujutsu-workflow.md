...
## git worktreeの代わりにもなる

並列開発といえば`git worktree`だけど、正直セットアップが面倒。

jujutsuには**ワークスペース機能**がある。`git worktree`と同じように複数の作業ディレクトリを持てるけど、もっとシンプル。

私はターミナル4並列でそのまま作業してるけど、ワークスペース機能を使えばさらに整理できる。モノレポでプロジェクト横断の変更も、変更追跡が自動だからストレスない。
